...
**An AI buying agent that can shop intelligently—without being trusted blindly.**
...
sequenceDiagram
    ...
    opt Approved and payment mode enabled
        RC->>S: Consume authorization artifact
        S->>RP: Create mock or Razorpay Test order
    end
```
...
- Optional Razorpay Test Mode adapter; live credentials are refused
...
| `RAZORPAY_WEBHOOK_SECRET` | Optional webhook-verification secret |
